...
VALUES (92,'Cardinal', 'Stavanger', 'Norway');
...
DELETE FROM Customers WHERE CustomerName='Tasty Tee';
...
--INNER JOIN(PRODUCT AND CATAGORY TABLE)
SELECT ProductID, ProductName, CategoryName
FROM Products
INNER JOIN Categories ON Products.CategoryID = Categories.CategoryID;

--LEFT JOIN(CUSTOMER AND ORDER TABLE)
...
--RIGHT JOIN(ORDERS AND EMPLOYEES)
SELECT Orders.OrderID, Employees.LastName, Employees.FirstName
FROM Orders
RIGHT JOIN Employees ON Orders.EmployeeID = Employees.EmployeeID
ORDER BY Orders.OrderID;

--SQL FULL OUTER JOIN(CUSTOMER AND ORDER TABLE)
SELECT Customers.CustomerName, Orders.OrderID
FROM Customers
FULL OUTER JOIN Orders ON Customers.CustomerID=Orders.CustomerID
ORDER BY Customers.CustomerName;

--SELF JOIN(CUSTOMER TABLE)
